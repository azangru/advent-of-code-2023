...
export const parseInput = (input: string): HandWithBid[] => {
  return input.trim()
    .split('\n')
    .map(parseLine);
};
...
export const interpretHand = (hand: string): Hand => {
...
const getHandType = (hand: number[]) => {
  ...
  const accValues = Object.values(acc).toSorted((num1, num2) => num2 - num1);
  accValues[0] = accValues[0] + jokersCount; // add all jokers to the most frequent card
